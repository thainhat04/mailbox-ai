  ...
  async testImapConnection(userId: string): Promise<{
    ...
    try {
      ...
      if (!tokens || tokens.length === 0) {
        return {
          success: false,
          message: 'No OAuth2 tokens found. Please connect your email account first.',
        };
      }
      ...
      this.logger.log(`Testing IMAP connection for ${token.email} (${token.provider})`);
      ...
      try {
        const testEmails = await this.imapService.fetchEmails(config, 'INBOX', 5);
      ...
      return {
        ...
        email: token.email,
        provider: token.provider,
        host: configDetails?.imapHost || (token.provider === MailProvider.GOOGLE ? 'imap.gmail.com' : 'outlook.office365.com'),
      ...
      this.logger.error(`IMAP connection test failed: ${error.message}`, error.stack);
      return {
        success: false,
